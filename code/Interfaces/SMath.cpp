...
SMath::vec2f SMath::normalize(SMath::vec2f vec)
{
    ...
    float length = SMath::fastSqrt(vec.x * vec.x + vec.y * vec.y);
    return SMath::vec2f(vec.x / length, vec.y / length);
}

float SMath::length(SMath::vec2f p1, SMath::vec2f p2)
{
    return SMath::fastSqrt((p1.x - p2.x) * (p1.x - p2.x) + (p1.y - p2.y) * (p1.y - p2.y));
}
...
{
    return SMath::fastSqrt(p.x * p.x + p.y * p.y);
}
...
{
    return SMath::vec2f(SMath::fast_cos(a), SMath::fast_sin(a));
}
...
float SMath::distToLine(SMath::vec2f p1, SMath::vec2f p2, SMath::vec2f p)
{
    ...
    float p_ = (a + b + c) / 2;
    float S = SMath::fastSqrt(p_ * (p_ - a) * (p_ - b) * (p_ - c));
    return 2 * S / c;
}
